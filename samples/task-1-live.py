...
def print_people():
    ...
    print_table('people', people_names)
...
def select_from_people_menu(message, people):
    print_menu(message, [person.name for person in people])
    selection = get_menu_input(f'{message} ')
    return validate_menu_input(selection, people)
...
# DONE Add a Person class
# Watch everything else break
# Fix eveything class
class Person:
    ...
    def can_legally_drink_alcohol(self):
        return self.age >= 18
...
def load_people(PEOPLE_FILE_PATH):
    for name in load_from_file(PEOPLE_FILE_PATH):
        people.append(Person(name, 18))
    # people_data_list = 
    # return people_data_list

def load_data():
    load_people(PEOPLE_FILE_PATH)
...
def save_people(path, people):
    people_names = [person.name for person in people]
    save_to_file(path, people_names)
...
def handle_set_favourite_drink():
    person = select_from_people_menu('Choose a person', people)
    wait()
    if person is False:
        wait()
        return

    drink = select_from_menu(f'Choose a drink for {person.name}', drinks)
    if drink is False:
        wait()
        return
...
def handle_exit():
    ...
    save_people(PEOPLE_FILE_PATH, people)
    save_favourites(favourite_drinks)
    print(f'Thank you for using {APP_NAME}')


# Handle making a round
    # Get a name from the user starting the round
        # Create a round (current_round = Round(name))
    # Some kind of loop
        # Get a name from the user
        # Get a drink from the user
            # Add a drink to our round (current_round.add_an_order(name, drink))
